...
class DiagMessageFixture : public component::ComponentTest {
 ...
 protected:
  ...
  auto CreateServerWithExpectation(Functor expectation_functor) noexcept -> std::future<bool> {
    return std::async(
        std::launch::async, [this, expectation_functor = std::move(expectation_functor)]() {
          ...
          if (server.has_value()) {
            ...
            common::ComponentTestLogger::GetComponentTestLogger().GetLogger().LogInfo(
                FILE_NAME, __LINE__, __func__,
                [](std::stringstream& msg) { msg << "Server created and expectation set "; });
          } else {
            common::ComponentTestLogger::GetComponentTestLogger().GetLogger().LogError(
                FILE_NAME, __LINE__, __func__,
                [](std::stringstream& msg) { msg << "Failed to get Tcp server "; });
          }
