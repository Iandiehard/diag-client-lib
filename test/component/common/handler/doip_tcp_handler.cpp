...
void DoipTcpHandler::ProcessReceivedTcpMessage(
  ...
  switch (doip_message.GetPayloadType()) {
    case kDoipRoutingActivationReqType: {
      ...
      ComponentTestLogger::GetComponentTestLogger().GetLogger().LogInfo(
          FILE_NAME, __LINE__, __func__,
          [](std::stringstream &msg) { msg << "Calling ProcessRoutingActivationRequestMessage "; });
      ProcessRoutingActivationRequestMessage(client_source_address, activation_type,
                                             std::move(vm_specific));
    } break;
