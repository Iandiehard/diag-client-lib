...
template<typename T>
auto AddCharToString(std::string_view input_string, T character,
                     std::uint8_t substring_range) noexcept -> std::string {
  std::string result{};
  std::uint8_t const string_length{static_cast<std::uint8_t>(input_string.length())};
  for (std::uint8_t char_count{0u}; char_count < string_length; char_count += substring_range) {
...
DoipUdpHandler::DoipUdpHandler(std::string_view broadcast_ip_address,
                               std::string_view unicast_ip_address, std::uint16_t local_port_number)
...
void DoipUdpHandler::Initialize() {
  udp_broadcast_server_.SetReadHandler([this](UdpServer::MessagePtr udp_message) {
    ProcessReceivedUdpMessage(std::move(udp_message));
  });
  udp_unicast_server_.SetReadHandler([this](UdpServer::MessagePtr udp_message) {
    ProcessReceivedUdpMessage(std::move(udp_message));
  });
...
void DoipUdpHandler::ProcessReceivedUdpMessage(DoipUdpHandler::UdpServer::MessagePtr udp_message) {
  message::DoipMessage doip_message{udp_message->GetHostIpAddress(),
                                    udp_message->GetHostPortNumber(), udp_message->GetPayload()};

  switch (doip_message.GetPayloadType()) {
    case kDoip_VehicleIdentification_ReqType:
      ProcessVehicleIdentificationRequestMessage(doip_message.GetHostIpAddress(),
                                                 doip_message.GetHostPortNumber(), {}, {});
      break;
    case kDoip_VehicleIdentificationEID_ReqType: {
      std::string const eid{
          AddCharToString(ConvertToHexString(doip_message.GetPayload()), ':', 2u)};
      ProcessVehicleIdentificationRequestMessage(doip_message.GetHostIpAddress(),
                                                 doip_message.GetHostPortNumber(), eid, {});
    } break;
    case kDoip_VehicleIdentificationVIN_ReqType: {
      std::string const vin{ConvertToAsciiString(doip_message.GetPayload())};
      ProcessVehicleIdentificationRequestMessage(doip_message.GetHostIpAddress(),
                                                 doip_message.GetHostPortNumber(), {}, vin);
    } break;
  }
}

void DoipUdpHandler::SendUdpMessage(
    DoipUdpHandler::UdpServer::MessageConstPtr udp_message) noexcept {
  EXPECT_TRUE(udp_unicast_server_.Transmit(std::move(udp_message)).HasValue());
}

auto DoipUdpHandler::ComposeVehicleIdentificationResponse(
    std::string_view remote_ip_address, std::uint16_t remote_port_number, std::string_view vin,
    std::uint16_t logical_address, std::string_view eid, std::string_view gid,
    std::uint8_t action_byte, std::optional<std::uint8_t> sync_status) noexcept
    -> UdpServer::MessagePtr {
  // Create header
  constexpr std::uint8_t kHeaderSize{8u};
  UdpServer::Message::BufferType response_buffer{
      CreateDoipGenericHeader(kDoip_VehicleAnnouncement_ResType, 32)};
  // Add VIN
  UdpServer::Message::BufferType const vin_in_bytes{SerializeToByteVectorFromString(vin, 1u)};
  response_buffer.insert(std::next(response_buffer.begin(), kHeaderSize), vin_in_bytes.cbegin(),
                         vin_in_bytes.cend());
  // Add LA
  response_buffer.emplace_back(logical_address >> 8U);
  response_buffer.emplace_back(logical_address & 0xFFU);
  // Add EID
  UdpServer::Message::BufferType const eid_in_bytes{
      SerializeToHexVectorFromString(RemoveCharFromString(eid, ':'), 2u)};
  constexpr std::uint8_t kEidOffset{kHeaderSize + 19u};
  response_buffer.insert(std::next(response_buffer.begin(), kEidOffset), eid_in_bytes.cbegin(),
                         eid_in_bytes.cend());
  // Add GID
  UdpServer::Message::BufferType const gid_in_bytes{
      SerializeToHexVectorFromString(RemoveCharFromString(gid, ':'), 2u)};
  constexpr std::uint8_t kGidOffset{kHeaderSize + 25u};
  response_buffer.insert(std::next(response_buffer.begin(), kGidOffset), gid_in_bytes.cbegin(),
                         gid_in_bytes.cend());
  ...
  UdpServer::MessagePtr response{std::make_unique<UdpServer::Message>(
      remote_ip_address, remote_port_number, std::move(response_buffer))};
  return response;
}
