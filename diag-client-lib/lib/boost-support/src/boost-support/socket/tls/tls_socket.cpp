...
TlsSocket::TlsSocket(std::string_view local_ip_address, std::uint16_t local_port_num,
...
core_type::Result<void, TlsSocket::SocketError> TlsSocket::Open() noexcept {
  ...
  if (ec.value() == boost::system::errc::success) {
    ...
    if (ec.value() == boost::system::errc::success) {
      ...
      common::logger::LibBoostLogger::GetLibBoostLogger().GetLogger().LogError(
          __FILE__, __LINE__, __func__, [ec](std::stringstream &msg) {
            msg << "Tls Socket binding failed with message: " << ec.message();
          });
    ...
    common::logger::LibBoostLogger::GetLibBoostLogger().GetLogger().LogError(
        __FILE__, __LINE__, __func__, [ec](std::stringstream &msg) {
          msg << "Tls Socket opening failed with error: " << ec.message();
        });
...
core_type::Result<void, TlsSocket::SocketError> TlsSocket::Connect(
  ...
  GetNativeTcpSocket().connect(
      Tcp::endpoint{boost::asio::ip::make_address(host_ip_address), host_port_num}, ec);
  if (ec.value() == boost::system::errc::success) {
    common::logger::LibBoostLogger::GetLibBoostLogger().GetLogger().LogDebug(
        __FILE__, __LINE__, __func__, [this](std::stringstream &msg) {
...
core_type::Result<void, TlsSocket::SocketError> TlsSocket::Transmit(
...
core_type::Result<TlsSocket::TcpMessagePtr, TlsSocket::SocketError> TlsSocket::Read() noexcept {
  ...
  boost::asio::read(ssl_stream_, boost::asio::buffer(&rx_buffer[0u], message::tcp::kDoipheadrSize),
                    ec);
  // Check for error
  if (ec.value() == boost::system::errc::success) {
...
TlsSocket::SslStream::lowest_layer_type &TlsSocket::GetNativeTcpSocket() {
