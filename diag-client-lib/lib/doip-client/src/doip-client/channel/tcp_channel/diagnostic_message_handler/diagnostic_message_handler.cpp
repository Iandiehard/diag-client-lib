...
auto DiagnosticMessageHandler::SendDiagnosticRequest(
  ...
  compose_diag_req.reserve(message::kDoipHeaderSize + total_diagnostic_response_length);
  ...
  // Copy data bytes
  compose_diag_req.insert(compose_diag_req.begin() + message::kDoipHeaderSize + kSourceAddressSize,
